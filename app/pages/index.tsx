...
export default function Page() {
  ...
  const handleSeekAttempt = () => { setSkipAttempts(s=>s+1); setError(`Seek blocked #${skipAttempts+1} logged`); setTimeout(()=>setError(''),3000); if(videoRef.current) videoRef.current.currentTime=0 }
  const handleSubmit = async () => {
    if(!answer.toLowerCase().includes('paralysis')){ setError(`Must type paralysis`); return }
