...
import { Client, TURNSTILE_OK, captureEmails, env, events, prescribe, verifiedPatient } from './helpers';
...
describe('patient one-time code', () => {
  ...
  it('blocks watching without a verified session', async () => {
    const s = await prescribe();
    const c = new Client();
    expect((await c.post(`/api/watch/${s.token}/video/${s.videoIds[0]}/playback`)).status).toBe(401);
  ...
  it('verifies the emailed code, opens a session and logs it', async () => {
    ...
    try {
      const c = new Client();
      expect((await c.post(`/api/watch/${s.token}/otp/send`, { turnstileToken: TURNSTILE_OK })).status).toBe(200);
  ...
  it('allows 5 guesses per code, then locks it even for the right code', async () => {
    ...
    try {
      const c = new Client();
      await c.post(`/api/watch/${s.token}/otp/send`, { turnstileToken: TURNSTILE_OK });
      ...
      expect((await c.fetch(`/api/watch/${s.token}/video/${s.videoIds[0]}/playback`, { method: 'POST' })).status).toBe(401);
    } finally {
      emails.restore();
    }
  });
  ...
  it('rejects a reused code, an expired code, and resends inside the cooldown', async () => {
    ...
    try {
      const c = new Client();
      await c.post(`/api/watch/${s.token}/otp/send`, { turnstileToken: TURNSTILE_OK });
      expect((await c.post(`/api/watch/${s.token}/otp/send`, { turnstileToken: TURNSTILE_OK })).status).toBe(429); // cooldown
      const code = codeFrom(emails.lastTo(s.patientEmail)!.text);
  ...
  it("doesn't let one prescription's session open another", async () => {
    ...
    expect((await patientA.fetch(`/api/watch/${b.token}/video/${b.videoIds[0]}/playback`, { method: 'POST' })).status).toBe(401);
  ...
  it('refuses expired and revoked links', async () => {
    ...
    expect((await patient.fetch(`/api/watch/${s.token}/video/${s.videoIds[0]}/playback`, { method: 'POST' })).status).toBe(410);
  ...
  it('requires a passing bot check before emailing a code', async () => {
    const s = await prescribe();
    const emails = captureEmails();
    try {
      const c = new Client();
      expect((await c.post(`/api/watch/${s.token}/otp/send`)).status).toBe(400);
      expect((await c.post(`/api/watch/${s.token}/otp/send`, { turnstileToken: 'forged' })).status).toBe(400);
      expect(emails.sent).toHaveLength(0);
      expect(emails.turnstile).toEqual(['forged']);
      expect((await c.post(`/api/watch/${s.token}/otp/send`, { turnstileToken: TURNSTILE_OK })).status).toBe(200);
